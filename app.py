...
from tools.pdf_search_tool import DocumentSearchTool
from tools.llm_chat_tool import start_llm_chat, send_llm_message
...
with st.sidebar:
    ...
    if selected == 'New Chat':
        with st.form(key='new_chat_form'):
            ...
            mode = st.selectbox('Select mode', options=['General Queries','Chat with PDF','Chat with Live Data','Chat with Live voice'])
            ...
            if submit_button and chat_title:
                ...
                st.session_state.messages = []
                if mode in ['General Queries', 'Chat with Live Data']:
                    st.session_state.gemini_history = []
                else:
        ...
        if st.session_state.mode in ['General Queries', 'Chat with Live Data']:
            try:
...
if 'chat_id' not in st.session_state:
    ...
    elif st.session_state.mode == 'General Queries':
        try:
    ...
    elif st.session_state.mode == 'Chat with Live voice':
        st.write("You will be redirected to the Finbot live website.")
        st.markdown("""
